...
#if WITH_WEBSOCKETS == WS_IS_WEBSOCKETS && LWS_LIBRARY_VERSION_NUMBER == 3002000
...
static int single_publish(struct mosquitto *context, struct mosquitto_message_v5 *msg, uint32_t message_expiry)
{
	...
	return db__message_insert(context, mid, mosq_md_out, (uint8_t)msg->qos, 0, stored, 0, true);
}
